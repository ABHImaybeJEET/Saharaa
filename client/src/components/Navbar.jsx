...
export default function Navbar({
  ...
  return (
    <header className="w-full bg-theme-card text-theme-primary border-b border-theme-border sticky top-0 z-[1000] px-2.5 sm:px-4 py-2 shadow-sm transition-colors duration-200">
      <div className="w-full max-w-[1700px] mx-auto flex items-center justify-between gap-1.5 sm:gap-3">
        
        {/* Left: Brand Logo & Grid Heartbeat */}
        <div 
          onClick={() => {
            soundEngine.playUiClick();
            setCurrentMode("landing");
          }}
          className="flex items-center space-x-1.5 sm:space-x-2 cursor-pointer select-none shrink-0"
        >
          ...
          <div className="flex items-center space-x-1.5">
            <span className="text-xs sm:text-sm font-black tracking-wider text-theme-primary font-sans">
              SAHARAA
            </span>
        ...
        {/* Center: Mode Switcher (Overview vs Authority vs Citizen) */}
        <div className="flex items-center bg-theme-subtle p-0.5 sm:p-1 rounded-xl border border-theme-border text-xs font-bold">
          <button
            onClick={() => {
              soundEngine.playUiClick();
              setCurrentMode("landing");
            }}
            className={`flex items-center space-x-1 px-2 sm:px-2.5 py-1 rounded-lg transition-all cursor-pointer ${
              currentMode === "landing"
          ...
          >
            ...
            <span className="text-[11px] sm:text-xs">Overview</span>
          </button>
          ...
          >
            ...
            <span className="text-[11px] sm:text-xs">Authority</span>
          </button>
          ...
          >
            ...
            <span className="text-[11px] sm:text-xs">Citizen</span>
          </button>
        </div>

        {/* Right Controls: Theme + Lang + Guide + Reset */}
